<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('fangwuxinxi','租用') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zuyongdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租用':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','租用') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zuyongdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租用':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','租用') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zuyongdingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'租用':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','保洁预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="baojieguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'保洁预约':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','保洁预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="baojieguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'保洁预约':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','保洁预约') && contents.tableBtnIcon == 0" type="success" size="mini" @click="baojieguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'保洁预约':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','安保预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="anbaoguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'安保预约':'' }}</el-button>
                <el-button v-if="isAuth('fangwuxinxi','安保预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="anbaoguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'安保预约':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('fangwuxinxi','安保预约') && contents.tableBtnIcon == 0" type="success" size="mini" @click="anbaoguanliCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'安保预约':'' }}</el-button>
    ...
    <zuyongdingdan-cross-add-or-update v-if="zuyongdingdanCrossAddOrUpdateFlag" :parent="this" ref="zuyongdingdanCrossaddOrUpdate"></zuyongdingdan-cross-add-or-update>
    <baojieguanli-cross-add-or-update v-if="baojieguanliCrossAddOrUpdateFlag" :parent="this" ref="baojieguanliCrossaddOrUpdate"></baojieguanli-cross-add-or-update>
    <anbaoguanli-cross-add-or-update v-if="anbaoguanliCrossAddOrUpdateFlag" :parent="this" ref="anbaoguanliCrossaddOrUpdate"></anbaoguanli-cross-add-or-update>
...
import zuyongdingdanCrossAddOrUpdate from "../zuyongdingdan/add-or-update";
import baojieguanliCrossAddOrUpdate from "../baojieguanli/add-or-update";
import anbaoguanliCrossAddOrUpdate from "../anbaoguanli/add-or-update";
export default {
  data() {
    return {
      leixingOptions: [],
      ...
      zuyongdingdanCrossAddOrUpdateFlag: false,
      baojieguanliCrossAddOrUpdateFlag: false,
      anbaoguanliCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    zuyongdingdanCrossAddOrUpdate,
    baojieguanliCrossAddOrUpdate,
    anbaoguanliCrossAddOrUpdate,
  },
  methods: {
    contentStyleChange() {
    ...
    zuyongdingdanCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.zuyongdingdanCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
      this.$refs.zuyongdingdanCrossaddOrUpdate.init(row.id,type);
      });
    },
    baojieguanliCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.baojieguanliCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
      this.$refs.baojieguanliCrossaddOrUpdate.init(row.id,type);
      });
    },
    anbaoguanliCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.anbaoguanliCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','fangwuxinxi');
      this.$nextTick(() => {
      this.$refs.anbaoguanliCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
